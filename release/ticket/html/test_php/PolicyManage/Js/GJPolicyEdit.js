...
function BindArea(table) {
    ...
    tree.find("s").click(function (e) {
        ...
        $.getJSON("GJPolicyEdit.php?Action=GetAirportList&AreaLevel=" + $(this).attr("level") + "&callback=?", function (data) {
            var htm = [];
...
function BerthType(d) {
    ...
    $.get("GJPolicyEdit.php?Action=GetBerthList&Carrier=" + carrier + "&Key=" + $(d).val(), function (data) {
        if (data != "") {
...
function GetAreasVal(name) {
    ...
    console.log("name: "+name+" chks: "+chks);
    var areas = [];
...
function BindEdit() {
    if ($("#hidID").val() != "") {
        ...
        $("#txtPCC").val(jsonEdit.pcc);
        setCheckbox("chkValidPlace", jsonEdit.validplace);
        ...
        //#region 时间
        //alert(jsonEdit.datelist.length);
...
function Validate(n) {
    switch (n) {
        case 0:
            ...
            var pcc = $("#txtPCC").val().toUpperCase();
            var validplace = getCheckbox("chkValidPlace");
            ...
            if (pcc=="" || pcc.length != 3) {
                alertPop("请正确填写PCC！", function () {
                    $("#txtPCC").focus();
                });
                return false;
            }
            if (validplace == "") {
                alertPop("请选择生效位置！", function () {
                    $("input[type='checkbox'][name='chkValidPlace']").focus();
                });
                return false;
            }
            //#endregion
...
function Save(d) {
    ...
    var voyagetype = getRadio("rdVoyage");
    var deplimit = getRadio("rdDepCitys");
    var depcitys = $("#txtDepCitys").val().toUpperCase();
    var depareas = GetAreasVal("tbDepArea");
    var arrlimit = getRadio("rdArrCitys");
    var arrcitys = $("#txtArrCitys").val().toUpperCase();
    var arrareas = GetAreasVal("tbArrArea");
    var backlimit = getRadio("rdBackCitys");
    var backcitys = $("#txtBackCitys").val().toUpperCase();
    var backareas = GetAreasVal("tbBackArea");
    ...
    $.post("GJPolicyEdit.php",
    { Action: "Save", ID: id,
      Carrier: carrier, CarrierSame: carriersame, OfficeProvider: provideoffice, OfficeGroupID: officegroupid, OfficeTicket: ticketoffice, TicketType: tickettype, TicketAuditing: ticketauditing, PCC: pcc, ValidPlace: validplace, AduType: adutype, ChdWay: chdway, ChdNone: chdnone, MixSeason: mixseason, MixBerth: mixberth, MixSB: mixsb, MixBase: mixbase, DateGroup: dategroup,
