    ...
    *lf->begin_code.str = LUA_SIGNATURE[0];
    ...
    if (lf->file_type == NGX_LUA_BT_LJ) {
        ...
        for (i = 0; i < LJ_HEADERSIZE; i++) {
            dd("%d: 0x%02X", i, (unsigned)(u_char)lf->begin_code.str[i]);
        }
        dd("==LJ_BT_HEADER_END==");
        }
#endif
        ...
        {
        size_t i = 0;
        dd("==LJ_END_CODE: %d rest_len: %d==", lf->end_code_len,
           lf->rest_len);

        for (i = 0; i < lf->end_code_len; i++) {
            dd("%d: 0x%02X", i, (unsigned) ((u_char) lf->end_code.ptr[i]));
        }
        dd("==LJ_END_CODE_END==");
        }
#endif
        ...
        *(lf->begin_code.str + POS_MAX_STACK_SIZE) = 2;
        ...
        {
        size_t i = 0;
        dd("==LUA_BEGIN_CODE: %d==", lf->begin_code_len);
        ...
        {
        size_t i = 0;
        dd("==LUA_END_CODE: %d==", lf->end_code_len);
        for (i = 0; i < lf->end_code_len; i++) {
            dd("%d: 0x%02X", i, (unsigned) ((u_char) lf->end_code.str[i]));
        }
        dd("==LUA_END_CODE_END==");
        }
#endif
