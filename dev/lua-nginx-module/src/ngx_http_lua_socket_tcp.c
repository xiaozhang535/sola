...
ngx_http_lua_inject_socket_tcp_api(ngx_log_t *log, lua_State *L)
{
    ...
    {
        const char  buf[] = "local sock = scan.socket.tcp()" //ginozhang
                            " local ok, err = sock:connect(...)"
                            " if ok then return sock else return nil, err end";

        rc = luaL_loadbuffer(L, buf, sizeof(buf) - 1, "=scan.socket.connect"); //ginozhang
    }

    if (rc != NGX_OK) {
        ngx_log_error(NGX_LOG_CRIT, log, 0,
                      "failed to load Lua code for scan.socket.connect(): %i", //ginozhang
                      rc);
